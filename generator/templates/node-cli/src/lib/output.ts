...
export function printError(error: { code: string; message: string; retry_after?: number }, format: OutputFormat): void {
  if (format === 'json') {
...
function formatCsv(rows: Record<string, unknown>[]): string {
  ...
  const keys = Object.keys(rows[0]);
  return stringify(rows.map((row) => keys.map((k) => row[k])), { header: true, columns: keys });
}
